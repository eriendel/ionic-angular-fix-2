(function (factory) {
    ...
    function getSegmentsFromNavGroups(navGroups, navLinks) {
        ...
        var _loop_1 = function (navGroup) {
            ...
            var matchedExactLink = navLinks.find(function (navLink) {
                return navLink.segmentParts.join('/') === segmentPieces.join('/');
            });
            var matchedMostLink = navLinks.map(function (navLink) {
                if (navLink.segmentParts.length === segmentPieces.length) {
                    var matchedRateLinkObj = { navLink: navLink, matchedExactlyPieces: 0 };
                    for (var idx = 0; idx < navLink.segmentParts.length; idx++) {
                        var segmentPart = navLink.segmentParts[idx];
                ...
                return null;
            }).filter(function (matchedLink) { return matchedLink; }).sort(function (l1, l2) { return l2.matchedExactlyPieces - l1.matchedExactlyPieces; })
                .map(function (matchedRateLinkObj) { return matchedRateLinkObj.navLink; })[0];
            if (matchedExactLink) {
                var segment = getSegmentsFromUrlPieces(segmentPieces, matchedExactLink);
                segments.push(segment);
            }
            else if (matchedMostLink) {
                var segment = getSegmentsFromUrlPieces(segmentPieces, matchedMostLink);
                segments.push(segment);
            }
            else {
